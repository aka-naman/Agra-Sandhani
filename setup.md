...
2.  **`node_modules`**: You must copy the `node_modules` folders in both `client/` and `server/` because `npm install` will not work offline.
    *   **CRITICAL**: Run the server (`npm start`) once on the internet-connected machine before moving it. This ensures the `node-llama-cpp` library downloads the necessary **offline binary** into `server/node_modules/node-llama-cpp/bins`.
3.  **AI Model**: Ensure `server/models/Qwen-2.5-Coder-3B-SQL-Writer.Q4_K_M.gguf` is present.
